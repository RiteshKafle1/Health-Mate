...
async def register_user(name: str, email: str, password: str) -> dict:
    ...
    user_data = {
        ...
        "last_profile_reminder_at": current_timestamp,
    ...
    # Send initial profile completion reminder if incomplete
    if completion < 100:
        try:
            await send_profile_completion_reminder(user_id)
        except Exception as e:
            print(f"Error sending initial profile reminder: {e}")
...
async def login_user(email: str, password: str) -> dict:
    ...
    if should_send_profile_reminder(user):
        try:
            await send_profile_completion_reminder(user_id)
        except Exception as e:
            print(f"Error sending profile reminder at login: {e}")
...
async def update_user_profile(
    ...
    """Update user profile."""
    from ..notification_service import calculate_profile_completion, create_notification
    ...
    if image_bytes:
        try:
            upload_result = await upload_image_from_bytes(image_bytes, "profile")
    ...
    if updated_user:
        ...
        previous_completion = updated_user.get("profile_completion_percentage", 0)
        ...
        # Send congratulations if profile just reached 100%
        if completion >= 100 and previous_completion < 100:
            try:
                await create_notification(
                    user_id=user_id,
                    notification_type="profile_incomplete",  # Reuse type
               ...
               print(f"Error sending profile completion congratulations: {e}")
