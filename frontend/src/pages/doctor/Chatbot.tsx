                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-400">AI Medical Assistant for Doctors</p>
                                </div>
                            </div>
                        </div>
                        <button
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {messages.length === 0 && (
                        <div className="h-full flex flex-col items-center justify-center text-center">
                            <div className="p-4 rounded-full bg-gradient-to-br from-indigo-500/20 to-purple-600/20 mb-4">
                            ...
                            <p className="text-slate-400 max-w-md">
                                Your AI-powered medical assistant. Get clinical insights, drug interactions,
                                treatment protocols, and evidence-based medical information.
                            </p>
                    ...
                    {messages.map((message, index) => (
                        ...
                        >
                            {message.role === 'assistant' && (
                                <div className="p-2 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 h-fit">
                            ...
                            >
                                ...
                                <div className={`flex items-center gap-2 mt-2 text-xs ${message.role === 'user' ? 'text-indigo-200' : 'text-slate-500'
                                    }`}>
                ...
                <form onSubmit={handleSendMessage} className="p-4 border-t border-slate-700/50 bg-slate-800/30">
                    <div className="flex gap-3">
                        <input
                            ...
                            placeholder="Ask about treatments, drug interactions, protocols..."
                            className="flex-1 px-4 py-3 rounded-xl bg-slate-700/50 border border-slate-600/50 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50"
                            disabled={isLoading}
                        />
                        <button
                            type="submit"
                            disabled={!inputMessage.trim() || isLoading}
                            className="px-6 py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-medium hover:from-indigo-600 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2"
                        >
                            <Send className="w-5 h-5" />
                        </button>
                    </div>
                    <p className="text-xs text-slate-500 mt-2 text-center">
                        HealthMate Clinician provides clinical decision support. Always verify with current guidelines.
                    </p>
                </form>
            </div>
        </div>
